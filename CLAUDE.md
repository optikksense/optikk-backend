...
- **Alerting**: `internal/modules/alerting/{rules,incidents,silences,slack,engine}` (plus existing `evaluators/`, `channels/` and the composition `factory/`) — `/api/v1/alerts/*`. Parent carries only shared types + helpers; each submodule is a proper directory with handler + service (+ repo for rules). `engine/` is the sole `BackgroundRunner` (evaluator loop + dispatcher + outbox relay + Redis lease). `engine/store.go` reads `spans_rollup` for SLO / service error rate, `spans_by_version` for deploy correlation; `queryAIMetric` stays on raw spans (cost_usd / quality.score attributes not materialized in `ai_spans_rollup`).
...
- **Schema migrations**: `db/clickhouse/*.sql` is embedded via `db/clickhouse/embed.go` and applied by `internal/infra/database/chmigrate`. CLI: `go run ./cmd/migrate up` (apply pending) or `go run ./cmd/migrate status` (list). Tracking table: `observability.schema_migrations`. Idempotent — re-runs skip already-applied files. Additive-only schema; there is no `down`.
- **CH rollup tables** (all AggregatingMergeTree, 1m/5m/1h cascade, 90-day TTL). Phase 5/6: `spans_rollup`, `metrics_histograms_rollup`, `logs_rollup`, `ai_spans_rollup`, `spans_error_fingerprint`, `spans_host_rollup`, `spans_by_version`. **Phase 7 additions**: `metrics_gauges_rollup` (service/host/pod/state_dim gauges for apm + httpmetrics + infrastructure), `metrics_gauges_by_status_rollup` (http req-rate by status code), `db_histograms_rollup` (saturation DB latency/volume/errors keyed on db_system/db_operation/db_collection/db_namespace/pool_name/error_type/server_address), `messaging_histograms_rollup` (saturation Kafka latency panels), `spans_topology_rollup` (service-to-service edges via `mat_peer_service` on CLIENT spans). **Phase 9 additions**: `metrics_k8s_rollup` (container + namespace + pod-phase state for infrastructure/kubernetes), `messaging_counters_rollup` (universal messaging metric rollup — counter + gauge + histogram rows, keys include broker + partition + error_type; backs all saturation/kafka non-latency panels), `spans_peer_rollup` (external-host / peer-service CLIENT-span aggregates for overview/httpmetrics external-host + status-distribution panels), `spans_kind_rollup` (span-kind breakdown). Phase 9 v2 extensions: `metrics_gauges_rollup_v2` (extended state_dim extractor: network.state, filesystem.mountpoint, jvm.memory.type, jvm.thread.daemon, jvm.buffer.pool.name, process.status, paging.state, db.client.connections.state), `db_histograms_rollup_v2` (adds db_connection_state + db_response_status_code keys + gauge-row state), `ai_spans_rollup_v2` (adds cost_usd + quality_score state columns for alerting AI rules). State columns via `quantilesTDigestWeighted` + `sum` + `any` + `min` / `max` + `argMax` (for gauge last-value). **Never emit `quantileExact*` or `uniqExact*` in SQL**; use the `*Merge` combinator on the rollup's state column (e.g. `quantilesTDigestWeightedMerge(0.5, 0.95, 0.99)(latency_ms_digest).2` for p95).
