	...
	for _, rl := range payload.ResourceLogs {
		...
		for _, sl := range rl.ScopeLogs {
			for _, record := range sl.LogRecords {
				...
				if traceID == "" {
					traceID = firstNonEmpty(logAttrs["trace.id"], logAttrs["trace_id"], logAttrs["otel.trace_id"])
				}
				spanID := strings.TrimSpace(record.SpanID)
				if spanID == "" {
					spanID = firstNonEmpty(logAttrs["span.id"], logAttrs["span_id"], logAttrs["otel.span_id"])
				}
