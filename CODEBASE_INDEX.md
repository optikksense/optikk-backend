...
- [trend/](internal/modules/traces/trend/) — `POST /api/v1/traces/trend`. Time-bucketed `(total, errors)` over the filtered window grouped by the window-adaptive display grain via `timebucket.DisplayGrainSQL(EndMs-StartMs)` (1m ≤3h, 5m ≤24h, 1h ≤7d, 1d beyond) on the `timestamp` column — so granularity tracks the selected range instead of the fixed 5-min `ts_bucket` (`ts_bucket` stays in PREWHERE for partition pruning). Reads `observability.spans_1m` with `spans_resource` CTE. Peer to `/traces/query` and `/traces/facets`.
...
- [errors/](internal/modules/services/errors/) — `GET /errors/{service-error-rate,error-volume,latency-during-error-windows,groups}`, `/errors/groups/:groupId{,/traces,/timeseries}`, `/spans/{exception-rate-by-type,error-hotspot,http-5xx-by-route}`, `/errors/fingerprints{,/trend}`. 12 repo methods, each with `*All` and `*ByService` siblings; reads `observability.spans` with `spans_resource` CTE. The group-detail response (`/errors/groups/:groupId`) projects `sample_stacktrace` (+ `sample_trace_id`, `exception_type`) for the FE stack-frames panel. `service-error-rate` and `latency-during-error-windows` share the same `ServiceErrorRateRows*` repo query — service.go discards zero-error rows for the latency view.
