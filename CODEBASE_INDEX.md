...
- [internal/modules/metrics/explorer/repository.go](internal/modules/metrics/explorer/repository.go) — every query is `const query = \`…\``, every query PREWHEREs the PK:
    - `ListMetricNames`: single statement with `WITH names AS (SELECT DISTINCT metric_name FROM observability.metrics_resource PREWHERE team_id = @teamID AND ts_bucket BETWEEN @bucketStart AND @bucketEnd WHERE metric_name ILIKE @search)` candidate-narrowing CTE → outer SELECT PREWHEREs raw `metrics` on `(team_id, ts_bucket_hour, metric_name IN names)` for type/unit/description lookup. Two-phase but one round-trip; the dictionary CTE is tiny.
    - `ListTagKeys`: PREWHEREs raw metrics on `(team_id, ts_bucket_hour, metric_name)`, then `mapKeys(JSONAllPathsWithTypes(attributes))` for dynamic-keys side; UNION ALL with constant resource-keys list.
    ...
    - `ListTagValues` data-point path: PREWHEREs raw metrics on `(team_id, ts_bucket_hour, metric_name)`, projects sanitized `attributes.\`<key>\`::String`.
    - `QueryTimeseries`: inline `WITH active_fps AS (SELECT fingerprint FROM observability.metrics_resource PREWHERE team_id = @teamID AND ts_bucket BETWEEN @bucketStart AND @bucketEnd AND metric_name = @metricName <resourceWhere>)` CTE → outer SELECT PREWHEREs raw metrics on `(team_id, ts_bucket_hour, fingerprint IN active_fps, metric_name)`. One round-trip per filtered query.
...
- **Resolver narrowing**: every query opens with `WITH active_fps AS (SELECT fingerprint FROM observability.metrics_resource WHERE team_id = @teamID AND ts_bucket BETWEEN @bucketStart AND @bucketEnd AND metric_name [= @metricName | IN @metricNames])`. Main query PREWHEREs `(team_id, ts_bucket_hour, fingerprint IN active_fps)` — first three slots of the metrics PK. Hour-aligned bounds via [timebucket.MetricsHourBucket](internal/infra/timebucket/timebucket.go). Spans-backed reads use the same shape with `spans_resource` and `timebucket.SpansBucketStart` (5-minute aligned), mirroring [httpmetrics.QueryRouteAgg](internal/modules/services/httpmetrics/repository.go#L143-L167).
...
- **Resource resolver**: every query opens with `WITH active_fps AS (SELECT fingerprint FROM observability.metrics_resource WHERE team_id = @teamID AND ts_bucket BETWEEN @bucketStart AND @bucketEnd AND metric_name [IN @metricNames | = @metricName])`. Main query PREWHEREs `(team_id, ts_bucket_hour, fingerprint IN active_fps)` — first three slots of the metrics PK — before any JSON-path read. Hour-aligned bounds come from [timebucket.MetricsHourBucket](internal/infra/timebucket/timebucket.go) (no CH-side `toStartOfHour`).
...
- **Source-table choice (per OTel semconv 1.30+)**:
  ...
  - **Metrics-side (`connections` only)** — the OTel `db.client.connection.*` family is instrumentation-side gauges/counters/histograms, not per-call spans. Reads `observability.metrics` with `WITH active_fps AS (SELECT fingerprint FROM observability.metrics_resource PREWHERE team_id = @teamID AND ts_bucket BETWEEN @bucketStart AND @bucketEnd AND metric_name [= @metricName | IN @metricNames])` + main scan PREWHERE on `(team_id, ts_bucket_hour, fingerprint IN active_fps, metric_name)`. 8 methods cover: count/max/idle.max/idle.min/pending_requests (gauges, raw `value`), timeouts (counter → service folds + per-second rate), wait_time/create_time/use_time (OTel native histograms with `hist_buckets` + `hist_counts`; service merges per `(display_bucket, pool.name)` and computes p50/p95/p99 via `quantile.FromHistogram`).
...
- **Metrics schema**: PK = `(team_id, ts_bucket_hour, fingerprint, metric_name, temporality, timestamp)` per [db/clickhouse/03_metrics.sql](db/clickhouse/03_metrics.sql); `metrics_resource` PK = `(team_id, ts_bucket, metric_name, fingerprint)` per [db/clickhouse/04_resources.sql](db/clickhouse/04_resources.sql).
