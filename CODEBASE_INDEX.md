...
| `arrayReduce('avg', arrayFilter(isNotNull, [...]))` | `internal/infra/stats.AvgNonNullPtr(...)` (or a repo-local `calculateAverage(nullableToSlice(...))` helper) in Go | Array materialization per row is wasteful; stdlib helper is 10 lines. Modules under the Phase-2 combinator-free policy (see below) **must** compute this Go-side rather than with `coalesce`/`nullIf`/`if` in SELECT. |
...
**Compute-in-service phase 2 (2026-04-19)** — `internal/modules/ai/explorer/`, `internal/modules/traces/query/`, `internal/modules/infrastructure/memory/`, `internal/modules/infrastructure/cpu/`, `internal/modules/infrastructure/disk/`, `internal/modules/infrastructure/network/`, `internal/modules/infrastructure/connpool/`, `internal/modules/infrastructure/kubernetes/`, `internal/modules/infrastructure/fleet/`, `internal/modules/overview/overview/`, `internal/modules/overview/slo/`, and the full `internal/modules/saturation/database/{system,systems,slowqueries,collection,connections,summary,volume,errors}/` tree are now combinator-free. No `-If` aggregators, no `multiIf` / `if(...)` in SELECT, no `toInt64 / toUInt* / toFloat* / toString` casts, no `groupUniqArray` / `arrayJoin` / `arrayStringConcat` / `arrayFilter` / `arrayReduce`, no `coalesce` / `nullIf` / `CASE WHEN` in SELECT, no `avg(col)`. Pattern: two-/three-scan `errgroup` split (totals + errors + optionally distinct-services), merged + divided in Go (mirrors `overview/errors`). AI session aggregation is now Go-side — the repo returns raw `aiSessionRawRow` and `service.aggregateSessions` does the non-empty-first session id pick, de-dup, argMax-by-timestamp, and pagination. Latency histogram + heatmap return the numeric `bucket_min`/`latency_bucket` value; the service formats the string label. Use `s.http_status_code` (UInt16 alias for `response_status_code`) in comparisons — never `toUInt16OrZero(response_status_code)`. Token attributes (`gen_ai.usage.*`) are extracted as strings via `JSONExtractString` and parsed Go-side (`parseFloatOrZero`); when aggregated SQL-side (summary/trend totals), use `JSONExtractFloat(s.attributes, 'key')` as a pure field extractor inside `sum(...)`.
