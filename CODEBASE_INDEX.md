...
| `observability.spans_resource` ([04_spans_resource.sql](db/clickhouse/04_spans_resource.sql)) | `(team_id, ts_bucket, fingerprint)` | `toYYYYMMDD(toDateTime(ts_bucket)) × hour` | 90 d | MV from `spans` |
| `observability.logs_resource` ([05_logs_resource.sql](db/clickhouse/05_logs_resource.sql)) | `(team_id, ts_bucket, fingerprint)` | `toYYYYMMDD(toDateTime(ts_bucket))` | 90 d | MV from `logs` |
| `observability.metrics_resource` ([06_metrics_resource.sql](db/clickhouse/06_metrics_resource.sql)) | `(team_id, fingerprint)` | `toYYYYMMDD(toDateTime(ts_bucket))` | 90 d | MV from `metrics` |
| `observability.trace_index` ([07_trace_index.sql](db/clickhouse/07_trace_index.sql)) | `(trace_id, team_id, ts_bucket, fingerprint, timestamp, span_id)` | `toYYYYMMDD(toDateTime(ts_bucket))` | 30 d | `spans_to_trace_index` MV from `spans` (spans-fed); reverse-key projection used by [trace_logs](internal/modules/logs/trace_logs/) and every trace-id-scoped reader in [traces/detail](internal/modules/traces/detail/), [traces/paths](internal/modules/traces/paths/), [traces/shape](internal/modules/traces/shape/), [traces/servicemap](internal/modules/traces/servicemap/) via inline `WITH trace_loc AS (...) ... PREWHERE (ts_bucket, fingerprint) IN ...` |
| `observability.metrics_1m` ([08_metrics_1m.sql](db/clickhouse/08_metrics_1m.sql)) | `(team_id, metric_name, ts_bucket, fingerprint, attr_hash, timestamp)` | `toYYYYMMDD(timestamp)` | 90 d | `metrics_1m_mv` from `metrics` |
...
Hot resource/span attributes are JSON typed-path subcolumns inside `attributes JSON(\`service.name\` LowCardinality(String), \`host.name\` LowCardinality(String), …, max_dynamic_paths=100)`. Type-hinted paths are stored as native subcolumns with column-equivalent read performance — no JSON parse per row. Reader-facing names live as zero-cost `ALIAS` columns: `operation_name` (= `name`), `start_time` (= `timestamp`), `duration_ms` (= `duration_nano/1e6`), `status` (= `status_code_string`), `http_status_code` (= `toUInt16OrZero(response_status_code)`), `is_error` (= `if(has_error OR toUInt16OrZero(response_status_code) >= 400, 1, 0)`), `is_root` (= `if(parent_span_id IN ('', '0000000000000000'), 1, 0)`). `fingerprint` is a real top-level `UInt64` column (computed FNV-64a hash of identity attributes) written by the mapper. `error_group_id` is a native `MATERIALIZED` column precomputed on insert using MD5/CityHash on the error fingerprint combination.
...
- [trends/](internal/modules/logs/trends/) (package `log_trends`) — `POST /api/v1/logs/summary` and `POST /api/v1/logs/trend` (peer endpoints; the old composite `/logs/trends` is removed — frontend fires both in parallel). `Summary` reads SQL-side `count() / countIf(severity_bucket >= 4) / countIf(severity_bucket = 3)` → `{total, errors, warns}` (FE severity chips). `Trend` groups by `timebucket.DisplayGrainSQL(window)` at display grain (1m / 5m / 1h / 1d) and returns **wide-format** per-bucket rows `{time_bucket, total, error (sev≥4), warn (sev=3), info (sev=2), debug (sev≤1)}` for the FE severity-stacked trend — payload size bounded regardless of window length. Both readers emit the `active_fps` CTE only when a resource filter is present.
...
- [explorer/](internal/modules/traces/explorer/) — Handles `POST /api/v1/traces/query` (list-only), `POST /api/v1/traces/facets` (top-N aggregates), `POST /api/v1/traces/trend` (time-bucketed total/errors), and `POST /api/v1/traces/suggest` (autocomplete). Standard 6-file module layout. Reads `observability.spans` (for queries/suggestions) and `observability.spans_1m` (for facets/trends).
- [detail/](internal/modules/traces/detail/) — `GET` per-trace drill-downs: `/traces/:id` (summary card via `GetTraceSummary`), `/traces/:id/spans`, `/traces/:id/span-events`, `/traces/:id/spans/:spanId/attributes`, and `/traces/:id/related`. Canonical 6-file layout. Trace-id-scoped point lookups resolving ts_bucket + fingerprint bounds via `trace_index` before scanning `observability.spans`.
- [paths/](internal/modules/traces/paths/) — `GET /traces/:id/critical-path` and `/traces/:id/error-path`. Trace-id-scoped queries executing DAG longest-path and error-chain walks.
- [servicemap/](internal/modules/traces/servicemap/) — `GET /traces/:id/service-map` and `/traces/:id/errors`. Trace-id-scoped queries returning topology nodes/edges and grouping exception errors.
- [filter/](internal/modules/traces/filter/) — typed `Filters` (`Services`/`Operations`/`SpanKinds`/`HTTPMethods`/`HTTPStatuses`/`Statuses`/`Environments`/`PeerServices`/`TraceID`/`MinDurationNs`/`MaxDurationNs`/`HasError` + `Exclude*` + `Attributes []AttrFilter`) + `AttrFilter` + `Validate()` + `BuildClauses(f)`. Shared clause builder consumed by the explorer submodule.

Trace-id-scoped queries match with a plain `trace_id = @traceID` row predicate; handlers reject an empty `:traceId` path param with 400. Each submodule repository binds args locally.
...
- [errors/](internal/modules/services/errors/) — `GET /errors/{service-error-rate,error-volume,latency-during-error-windows,groups}`, `/errors/groups/:groupId{,/traces,/timeseries,/latest-occurrence,/facets}`, `/spans/{exception-rate-by-type,error-hotspot,http-5xx-by-route}`, `/errors/fingerprints{,/trend}`. Reads `observability.spans` with `spans_resource` CTE. The group-detail response (`/errors/groups/:groupId`) is now group-level only (`exception_type`, `http_status_code`, counts, first/last occurrence) — per-occurrence data moved to **`/errors/groups/:groupId/latest-occurrence`** (`ErrorGroupLatestOccurrenceRow`, raw-spans `ORDER BY timestamp DESC LIMIT 1`: message, stacktrace, trace_id/span_id, http method/route/status, duration, service_version/env/pod/host) which feeds the FE error banner, stack-frames panel, "Open trace", and request-context card. **`/errors/groups/:groupId/facets`** (`ErrorGroupFacetRows` + `facetColumns` whitelist) returns per-dimension `sum(error_count)` distributions (`service_version`/`environment`/`pod`/`http_route`) for the "Where it happens" panel. `service-error-rate` and `latency-during-error-windows` share the same `ServiceErrorRateRows*` repo query — service.go discards zero-error rows for the latency view.
...
- [redmetrics/](internal/modules/services/redmetrics/) — RED summary + Apdex + per-route + breakdowns. 12 repo methods. Latency-percentile methods (`GetSummary`, `GetTopSlowOperations`, `GetP95LatencyTimeSeries`, `GetOperationBaseline`) read `observability.spans_1m` and project `quantileTimingMerge(q)(latency_state)`; `GET /spans/red/operation-baseline?service=&operation=` returns windowed p50/p95/p99 for one service+operation (feeds the Trace Detail Duration "N× slower than p50" card); Apdex / error-by-route / span-kind / etc. read raw `observability.spans` (Apdex computes exact `countIf(duration_nano <= @satisfiedNs)` / `countIf(duration_nano > @satisfiedNs AND duration_nano <= @toleratingNs)` buckets, which require raw-span granularity). `*ByService` split (no inline `query += ...`). The service-level error-rate timeseries lives at `/errors/service-error-rate` (services/errors), not under `/spans/red/`.
...
**Saturation was simplified (2026-06)** to match the new design handoff: the Kafka hub is now a single overview (no tabs/drill-downs), so the unused panel routes — **and their handler/service/repo/DTO code** — were fully removed (not just unregistered). The `client/` submodule was deleted entirely. The design's fleet map / most-saturated-hosts / broker-CPU grid is backed by [infrastructure/hosts](internal/modules/infrastructure/hosts/) (`GET /infrastructure/hosts`), not a saturation submodule.
...
- [explorer/](internal/modules/saturation/kafka/explorer/) — 7 live catalog routes: `/saturation/kafka/{topics/throughput, topics/lag, topics/consumers, groups/partitions, groups/commits, groups/fetches, groups/health}`. The 3 detail-intersection routes (`topic/groups/throughput`, `topic/groups/lag`, `group/topics`) were removed. The frontend folds the surviving topic/group domain rows into `KafkaTopicRow` / `KafkaGroupRow`.
...
**Simplified (2026-06):** the Database hub is now a single overview serving exactly four routes — `explorer` `/saturation/datastores/systems`, `latency` `/latency/by-system`, `volume` `/ops/by-system`, `slowqueries` `/slow-queries/patterns` — and the unused routes + their handler/service/repo/DTO code were fully removed. `collection`, `connections`, `errors`, `system`, `summary` modules were **deleted entirely**. `explorer` was slimmed to a thin composer (its 7 datastore-detail handlers + the multi-service composition are gone). The former `systems` library was **merged into `explorer`** (2026-06) — it was single-use, so `explorer` now owns its two queries directly instead of wrapping a separate package.
...
- **services**: `deployments`, `errors`, `redmetrics`, `topology` (per-host views moved to `infrastructure/hosts`; the `latency` module — `/services/latency/{histogram,heatmap}` — was removed: no FE consumer, no design panel)
- **traces**: `explorer`, `detail`, `span_query`, `facets`, `paths`, `servicemap`, `shape`, `suggest`, `trend` (+ `filter` shared package). `detail` no longer exposes `GET /spans/:spanId/tree` (`ListSpanSubtree` removed — unused, no FE method); `GET /traces/:traceId` summary is kept for direct deep-links.
...
52 modules total (the `services/latency` module was removed).
