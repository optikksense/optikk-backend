...
Settings ride on the ctx via `clickhouse.Context(parent, clickhouse.WithSettings(...))`. Tests in `tests/database/budget_test.go` assert against `database.OverviewSettings()` / `database.ExplorerSettings()`.
