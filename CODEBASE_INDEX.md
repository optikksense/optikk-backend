...
| `observability.metrics_resource` ([06_metrics_resource.sql](db/clickhouse/06_metrics_resource.sql)) | `(team_id, ts_bucket, metric_name, fingerprint)` | `toYYYYMMDD(toDateTime(ts_bucket))` | 90 d | MV from `metrics` |
...
| `observability.metrics_1m` ([08_metrics_1m.sql](db/clickhouse/08_metrics_1m.sql)) | `(team_id, ts_bucket, fingerprint, metric_name, attr_hash, timestamp)` | `toYYYYMMDD(timestamp)` | 90 d | `metrics_1m_mv` from `metrics` |
