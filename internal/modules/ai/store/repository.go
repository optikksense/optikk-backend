...
func (r *ClickHouseRepository) GetAISummary(teamUUID string, startMs, endMs int64) (*model.AISummary, error) {
...
func (r *ClickHouseRepository) GetAIModels(teamUUID string, startMs, endMs int64) ([]model.AIModel, error) {
	...
	models := make([]model.AIModel, len(rows))
	for i, row := range rows {
		models[i] = model.AIModel{
...
func (r *ClickHouseRepository) GetAIPerformanceMetrics(teamUUID string, startMs, endMs int64) ([]model.AIPerformanceMetric, error) {
	rows, err := dbutil.QueryMaps(r.db, `
	...
	metrics := make([]model.AIPerformanceMetric, len(rows))
	for i, row := range rows {
		metrics[i] = model.AIPerformanceMetric{
			ModelName:       dbutil.StringFromAny(row["model_name"]),
			ModelProvider:   dbutil.StringFromAny(row["model_provider"]),
...
func (r *ClickHouseRepository) GetAIPerformanceTimeSeries(teamUUID string, startMs, endMs int64) ([]model.AIPerformanceTimeSeries, error) {
	...
	timeseries := make([]model.AIPerformanceTimeSeries, len(rows))
	for i, row := range rows {
		timeseries[i] = model.AIPerformanceTimeSeries{
			ModelName:    dbutil.StringFromAny(row["model_name"]),
...
func (r *ClickHouseRepository) GetAILatencyHistogram(teamUUID string, modelName string, startMs, endMs int64) ([]model.AILatencyHistogram, error) {
	query := `
		SELECT model_name,
	...
	histogram := make([]model.AILatencyHistogram, len(rows))
	for i, row := range rows {
		histogram[i] = model.AILatencyHistogram{
			ModelName:    dbutil.StringFromAny(row["model_name"]),
...
func (r *ClickHouseRepository) GetAICostMetrics(teamUUID string, startMs, endMs int64) ([]model.AICostMetric, error) {
	rows, err := dbutil.QueryMaps(r.db, `
	...
	metrics := make([]model.AICostMetric, len(rows))
	for i, row := range rows {
		metrics[i] = model.AICostMetric{
			ModelName:             dbutil.StringFromAny(row["model_name"]),
			ModelProvider:         dbutil.StringFromAny(row["model_provider"]),
			TotalRequests:         dbutil.Int64FromAny(row["total_requests"]),
			TotalCostUsd:          dbutil.Float64FromAny(row["total_cost_usd"]),
...
func (r *ClickHouseRepository) GetAICostTimeSeries(teamUUID string, startMs, endMs int64) ([]model.AICostTimeSeries, error) {
	...
	timeseries := make([]model.AICostTimeSeries, len(rows))
	for i, row := range rows {
		timeseries[i] = model.AICostTimeSeries{
			ModelName:        dbutil.StringFromAny(row["model_name"]),
			Timestamp:        dbutil.StringFromAny(row["timestamp"]),
...
func (r *ClickHouseRepository) GetAITokenBreakdown(teamUUID string, startMs, endMs int64) ([]model.AITokenBreakdown, error) {
	rows, err := dbutil.QueryMaps(r.db, `
		SELECT model_name,
		       SUM(COALESCE(tokens_prompt,0)) as prompt_tokens,
		       SUM(COALESCE(tokens_completion,0)) as completion_tokens,
	...
	breakdown := make([]model.AITokenBreakdown, len(rows))
	for i, row := range rows {
		breakdown[i] = model.AITokenBreakdown{
			ModelName:        dbutil.StringFromAny(row["model_name"]),
			PromptTokens:     dbutil.Int64FromAny(row["prompt_tokens"]),
			CompletionTokens: dbutil.Int64FromAny(row["completion_tokens"]),
...
func (r *ClickHouseRepository) GetAISecurityMetrics(teamUUID string, startMs, endMs int64) ([]model.AISecurityMetric, error) {
	rows, err := dbutil.QueryMaps(r.db, `
		SELECT model_name, model_provider,
		       COUNT(*) as total_requests,
	...
	metrics := make([]model.AISecurityMetric, len(rows))
	for i, row := range rows {
		metrics[i] = model.AISecurityMetric{
			ModelName:             dbutil.StringFromAny(row["model_name"]),
			ModelProvider:         dbutil.StringFromAny(row["model_provider"]),
			TotalRequests:         dbutil.Int64FromAny(row["total_requests"]),
...
func (r *ClickHouseRepository) GetAISecurityTimeSeries(teamUUID string, startMs, endMs int64) ([]model.AISecurityTimeSeries, error) {
	...
	timeseries := make([]model.AISecurityTimeSeries, len(rows))
	for i, row := range rows {
		timeseries[i] = model.AISecurityTimeSeries{
			ModelName:          dbutil.StringFromAny(row["model_name"]),
			Timestamp:          dbutil.StringFromAny(row["timestamp"]),
			TotalRequests:      dbutil.Int64FromAny(row["total_requests"]),
...
func (r *ClickHouseRepository) GetAIPiiCategories(teamUUID string, startMs, endMs int64) ([]model.AIPiiCategory, error) {
	rows, err := dbutil.QueryMaps(r.db, `
	...
	categories := make([]model.AIPiiCategory, len(rows))
	for i, row := range rows {
		categories[i] = model.AIPiiCategory{
			ModelName:      dbutil.StringFromAny(row["model_name"]),
