...
// Analytics handles POST /v1/explorer/traces/analytics
func (h *Handler) Analytics(c *gin.Context) {
	...
	result, err := runTracesAnalytics(c.Request.Context(), h.db, h.GetTenant(c).TeamID, req)
	if err != nil {
...
func runTracesAnalytics(ctx context.Context, db *dbutil.NativeQuerier, teamID int64, req analytics.AnalyticsRequest) (*analytics.AnalyticsResult, error) {
	...
	if req.Query != "" {
		...
		if node != nil {
			compiled, compileErr := queryparser.Compile(node, queryparser.TracesSchema{})
			if compileErr != nil {
