...
func (s *TraceDetailService) GetLLMTrace(teamID int64, traceID string) ([]LLMTraceSpan, error) {
	rows, err := s.repo.GetLLMTraceSpans(context.Background(), teamID, traceID)
	if err != nil {
		return nil, err
	}
...
func (s *TraceDetailService) GetLLMTraceSummary(teamID int64, traceID string) (*LLMTraceSummary, error) {
	rows, err := s.repo.GetLLMTraceSpans(context.Background(), teamID, traceID)
	if err != nil {
		return nil, err
	}
...
func buildTraceSpans(rows []llmTraceSpanDTO) []LLMTraceSpan {
