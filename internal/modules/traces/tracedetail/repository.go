...
// `service + name` (hitting `idx_service` + `idx_span_name`). With
...
//     spans rollup collapses (service, operation_name) ↦ percentile
...
// Phase 7: reads from observability.logs — a per-trace
...
func (r *ClickHouseRepository) GetSpanAttributes(ctx context.Context, teamID int64, traceID, spanID string) (*spanAttributeRow, error) {
	...
	if err := dbutil.SelectCH(dbutil.ExplorerCtx(ctx), r.db, "tracedetail.GetSpanAttributes", &rows, `
		SELECT s.span_id, s.trace_id, s.name AS operation_name, s.service,
...
// Reads raw `observability.spans` with PREWHERE on
// (team_id, service, name) — the planned `root_spans_index` summary
// table was never built; the bloom filters on `service` (via the JSON
// typed path) and `idx_span_name` keep the scan narrow.
func (r *ClickHouseRepository) GetRelatedTraces(ctx context.Context, teamID int64, serviceName, operationName string, startMs, endMs int64, excludeTraceID string, limit int) ([]RelatedTrace, error) {
	...
	err := dbutil.SelectCH(dbutil.ExplorerCtx(ctx), r.db, "tracedetail.GetRelatedTraces", &rows, `
		...
		PREWHERE team_id = @teamID AND service = @serviceName AND name = @operationName
		WHERE is_root = 1
