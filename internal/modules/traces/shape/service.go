...
func (s *service) GetSpanKindBreakdown(ctx context.Context, teamID int64, traceID string) ([]SpanKindDuration, error) {
	...
	if err != nil {
		slog.ErrorContext(ctx, "shape: GetSpanKindBreakdown failed", slog.Any("error", err), slog.Int64("team_id", teamID), slog.String("trace_id", traceID))
		return nil, err
	}
	var totalMs float64
	...
	for i, row := range rows {
		...
		result[i] = SpanKindDuration{SpanKind: row.SpanKind, TotalDuraMs: row.TotalDuraMs, SpanCount: int64(row.SpanCount)} //nolint:gosec // domain-bounded
	}
	if totalMs > 0 {
		for i := range result {
...
func (s *service) GetFlamegraphData(ctx context.Context, teamID int64, traceID string) ([]FlamegraphFrame, error) {
	...
	if err != nil {
		slog.ErrorContext(ctx, "shape: GetFlamegraphData failed", slog.Any("error", err), slog.Int64("team_id", teamID), slog.String("trace_id", traceID))
		return nil, err
	}
