// Package shape exposes the "shape/distribution of a trace" endpoints
// (flamegraph, span-kind breakdown) split out of the legacy detail module.
// Self-times are computed on the frontend from the span list returned by
// /traces/:id/spans — no server-side endpoint.
package shape
