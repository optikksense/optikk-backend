...
func (r *ClickHouseRepository) GetDeployments(teamUUID string, startMs, endMs int64, serviceName, environment string, limit, offset int) ([]Deployment, int64, error) {
	query := `
...
func (r *ClickHouseRepository) GetDeploymentEvents(teamUUID string, startMs, endMs int64, serviceName string) ([]DeploymentEvent, error) {
	query := `
	...
	events := make([]DeploymentEvent, len(rows))
	for i, row := range rows {
...
func (r *ClickHouseRepository) GetDeploymentDiff(teamUUID string, deployID string, windowMinutes int) (*DeploymentDiff, error) {
...
func rowToDeployment(row map[string]any) Deployment {
...
func rowToDeploymentEvent(row map[string]any) DeploymentEvent {
