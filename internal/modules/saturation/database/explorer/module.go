...
func (m *saturationExplorerModule) configure(deps *registry.Deps) {
	nq := deps.NativeQuerier
	m.handler = &Handler{
		DBTenant: modulecommon.DBTenant{GetTenant: deps.GetTenant},
		Service: NewService(
			dbsummary.NewService(dbsummary.NewRepository(nq)),
			dbsystems.NewService(dbsystems.NewRepository(nq)),
			dbsystem.NewService(dbsystem.NewRepository(nq)),
			dblatency.NewService(dblatency.NewRepository(nq)),
			dbvolume.NewService(dbvolume.NewRepository(nq)),
			dberrors.NewService(dberrors.NewRepository(nq)),
			dbslowqueries.NewService(dbslowqueries.NewRepository(nq)),
			dbconnections.NewService(dbconnections.NewRepository(nq)),
			saturationkafka.NewService(saturationkafka.NewRepository(nq)),
		),
	}
}
