...
type Repository interface {
	...
	GetAlertsPaged(teamID int64, limit, offset int) ([]Alert, int64, error)
	...
	GetIncidents(teamID int64, startMs, endMs int64, statuses, severities, services []string, limit, offset int) ([]Incident, int64, map[string]int64, map[string]int64, error)
...
func mapToAlert(row map[string]any) Alert {
...
func mapRowsToAlerts(rows []map[string]any) []Alert {
...
func (r *ClickHouseRepository) GetAlerts(teamID int64, status string) ([]Alert, error) {
...
func (r *ClickHouseRepository) GetAlertsPaged(teamID int64, limit, offset int) ([]Alert, int64, error) {
	query := r.selectSQL() + `
		WHERE team_id = ?
...
func (r *ClickHouseRepository) GetAlertByID(id int64) (*Alert, error) {
...
func (r *ClickHouseRepository) BulkMuteAlerts(ids []int64, minutes int, reason string) ([]Alert, error) {
	if len(ids) == 0 {
		return nil, nil
	}
	now := time.Now().UTC()
...
func (r *ClickHouseRepository) BulkResolveAlerts(ids []int64, resolvedBy string) ([]Alert, error) {
	if len(ids) == 0 {
		return nil, nil
	}
	now := time.Now().UTC()
...
func (r *ClickHouseRepository) GetAlertsForIncident(teamID int64, policyID string) ([]Alert, error) {
...
func (r *ClickHouseRepository) GetIncidents(teamID int64, startMs, endMs int64, statuses, severities, services []string, limit, offset int) ([]Incident, int64, map[string]int64, map[string]int64, error) {
	queryBase := `
		FROM alerts
		WHERE team_id = ?
