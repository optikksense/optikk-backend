...
-- v2 at bottom of file adds cost_usd + quality_score state columns (used by
-- alerting/engine queryAIMetric).
...
-- ---------------------------------------------------------------------------
-- ai_spans_rollup_v2 — superset of v1 adding cost_usd + quality_score state.
-- Consumers migrate from v1 → v2 as they need cost / quality aggregates.
-- v1 is retained during migration window and dropped after 90-day TTL passes.
-- ---------------------------------------------------------------------------

CREATE TABLE IF NOT EXISTS observability.ai_spans_rollup_v2_1m (
    team_id        UInt32 CODEC(T64, ZSTD(1)),
    bucket_ts      DateTime CODEC(DoubleDelta, LZ4),
    ai_system      LowCardinality(String),
    ai_model       LowCardinality(String),
    ...
    quality_score_sum      AggregateFunction(sum, Float64),
    quality_score_count    AggregateFunction(sum, UInt64)
) ENGINE = AggregatingMergeTree()
PARTITION BY toYYYYMM(bucket_ts)
ORDER BY (team_id, bucket_ts, ai_system, ai_model, ai_operation, service_name)
TTL bucket_ts + INTERVAL 90 DAY DELETE
...
CREATE MATERIALIZED VIEW IF NOT EXISTS observability.ai_spans_to_rollup_v2_1m
TO observability.ai_spans_rollup_v2_1m AS
SELECT
    team_id,
    toStartOfMinute(timestamp)                                                                 AS bucket_ts,
    attributes.`gen_ai.system`::String                                                         AS ai_system,
    ...
    sumState(toUInt64OrZero(attributes.`gen_ai.usage.input_tokens`::String))                   AS input_tokens_sum,
    sumState(toUInt64OrZero(attributes.`gen_ai.usage.output_tokens`::String))                  AS output_tokens_sum,
    sumState(toFloat64OrZero(attributes.`optikk.ai.cost_usd`::String))                         AS cost_usd_sum,
    sumState(toFloat64OrZero(attributes.`optikk.ai.quality.score`::String))                    AS quality_score_sum,
    sumState(toUInt64(toFloat64OrZero(attributes.`optikk.ai.quality.score`::String) > 0))      AS quality_score_count
FROM observability.spans
WHERE attributes.`gen_ai.system`::String != '';

CREATE TABLE IF NOT EXISTS observability.ai_spans_rollup_v2_5m (
    team_id        UInt32 CODEC(T64, ZSTD(1)),
    bucket_ts      DateTime CODEC(DoubleDelta, LZ4),
    ai_system      LowCardinality(String),
    ai_model       LowCardinality(String),
...
CREATE MATERIALIZED VIEW IF NOT EXISTS observability.ai_spans_rollup_v2_1m_to_5m
TO observability.ai_spans_rollup_v2_5m AS
SELECT team_id,
       toStartOfInterval(bucket_ts, toIntervalMinute(5)) AS bucket_ts,
       ai_system, ai_model, ai_operation, service_name,
       quantilesTDigestWeightedMergeState(0.5, 0.95, 0.99)(latency_ms_digest) AS latency_ms_digest,
       ...
       sumMergeState(input_tokens_sum)    AS input_tokens_sum,
       sumMergeState(output_tokens_sum)   AS output_tokens_sum,
       sumMergeState(cost_usd_sum)        AS cost_usd_sum,
       sumMergeState(quality_score_sum)   AS quality_score_sum,
       sumMergeState(quality_score_count) AS quality_score_count
FROM observability.ai_spans_rollup_v2_1m
GROUP BY team_id, bucket_ts, ai_system, ai_model, ai_operation, service_name;

CREATE TABLE IF NOT EXISTS observability.ai_spans_rollup_v2_1h (
    team_id        UInt32 CODEC(T64, ZSTD(1)),
    bucket_ts      DateTime CODEC(DoubleDelta, LZ4),
    ai_system      LowCardinality(String),
    ai_model       LowCardinality(String),
...
CREATE MATERIALIZED VIEW IF NOT EXISTS observability.ai_spans_rollup_v2_5m_to_1h
TO observability.ai_spans_rollup_v2_1h AS
SELECT team_id,
       toStartOfHour(bucket_ts) AS bucket_ts,
       ai_system, ai_model, ai_operation, service_name,
       quantilesTDigestWeightedMergeState(0.5, 0.95, 0.99)(latency_ms_digest) AS latency_ms_digest,
       ...
       sumMergeState(quality_score_sum)   AS quality_score_sum,
       sumMergeState(quality_score_count) AS quality_score_count
FROM observability.ai_spans_rollup_v2_5m
GROUP BY team_id, bucket_ts, ai_system, ai_model, ai_operation, service_name;
