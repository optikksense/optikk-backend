-- metrics_gauges_rollup — unified gauge + counter metric rollup (single cascade).
-- Covers infrastructure/{cpu,disk,memory,network,jvm,kubernetes,connpool},
-- overview/apm process panels, overview/httpmetrics active-request gauges.
-- state_dim holds per-metric-family attributes (cpu.state, memory.state, disk.direction,
-- network direction/state, jvm pool+type, db.client.connections.state, …) or ''.
--
-- Previously split v1/v2; v2’s extended multiIf is canonical. For
-- system.network.connections, state_dim prefers system.network.state and falls back
-- to system.network.direction so direction-only series remain addressable.
