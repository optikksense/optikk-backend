-- db_conn_pool — narrow rollup for /saturation/datastores/system/connections
-- endpoints. Collapses 13-dim db_saturation to 5 dims: (team_id, bucket_ts,
-- pool_name, metric_name, db_connection_state). All 8 connection panel
-- queries read this instead of the wide rollup.
...
ORDER BY (team_id, bucket_ts, pool_name, metric_name, db_connection_state)
...
SELECT
    team_id, bucket_ts, pool_name, metric_name, db_connection_state,
...
GROUP BY team_id, bucket_ts, pool_name, metric_name, db_connection_state;
...
SELECT
    ...
    pool_name, metric_name, db_connection_state,
    sumMergeState(value_sum)     AS value_sum,
    sumMergeState(sample_count)  AS sample_count,
    quantilesTDigestWeightedMergeState(0.5, 0.95, 0.99)(latency_ms_digest) AS latency_ms_digest
