-- spans_v2 — rewrite of observability.spans. Fixes the dev-leak 1-HOUR TTL
-- (see 01_spans.sql:114) to 30 DAYS, switches from monthly to daily
-- partitioning with ttl_only_drop_parts=1 for cheap partition-level drops,
-- lifts the JSON dynamic-paths cap 50 -> 100 (parity with logs_v2), and adds
-- two new skip indexes: set(2) on has_error for fast error-only scans, and a
-- tokenbf_v1 on trace_id to support partial trace-ID typeahead.
-- ORDER BY is preserved verbatim so the downstream rollup MVs keep their
-- cascade semantics.

CREATE TABLE IF NOT EXISTS observability.spans_v2 (
