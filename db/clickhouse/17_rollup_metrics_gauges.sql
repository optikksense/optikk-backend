-- metrics_gauges_rollup — generic gauge + counter metric rollup.
...
-- state_dim holds the per-metric-family state attribute (cpu.state, memory.state,
-- disk.direction, network.direction, jvm.memory.pool.name, jvm.gc.name, …) or
-- empty when the metric has no state dimension.
