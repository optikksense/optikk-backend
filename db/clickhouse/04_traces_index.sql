...
-- Explorer trace-list reads from here instead of raw spans, cutting row count
-- by ~2 orders of magnitude. The indexer has at-least-once semantics (trace
-- rows can be re-emitted as late spans arrive), so the engine is
-- ReplacingMergeTree keyed on (team_id, trace_id) with `last_seen_ms` as the
-- version column.
...
CREATE TABLE IF NOT EXISTS observability.traces_index (
    ...
    -- http_status_bucket: computed once here; lets spans_red MV stay pure
    -- GROUP BY + state. Mirrors the bucket on raw spans.
    http_status_bucket LowCardinality(String) MATERIALIZED multiIf(
        root_http_status >= 500, '5xx',
        root_http_status >= 400, '4xx',
        root_http_status >= 300, '3xx',
        root_http_status >= 200, '2xx',
        has_error, 'err',
        'other'
    ) CODEC(ZSTD(1)),
    ...
    -- by_start_desc projection: declared inline so list-by-time queries
    -- (POST /traces/query) keyset-scan the latest N traces by start_ms
    -- without a wide root_service-led scan. New parts inherit the
    -- projection at insert; on stage there is no historical data to
    -- backfill, so no MATERIALIZE PROJECTION step is needed.
    PROJECTION by_start_desc (
        SELECT *
