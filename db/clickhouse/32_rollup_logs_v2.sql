-- logs_rollup_v2_{1m, 5m, 1h} — v2 log-volume + error counts per
-- (severity_bucket, service, environment, host). `pod` kept only at the 1m
-- tier where its cardinality is tolerable; dropped on 5m/1h. Cascade MVs
-- preserved (raw → 1m, 1m → 5m, 5m → 1h). TTL 90 days on all tiers.

CREATE TABLE IF NOT EXISTS observability.logs_rollup_v2_1m (
...
CREATE MATERIALIZED VIEW IF NOT EXISTS observability.logs_v2_to_rollup_1m
TO observability.logs_rollup_v2_1m AS
SELECT
    team_id,
...
CREATE TABLE IF NOT EXISTS observability.logs_rollup_v2_5m (
    team_id         UInt32 CODEC(T64, ZSTD(1)),
    bucket_ts       DateTime CODEC(DoubleDelta, LZ4),
    severity_bucket UInt8,
    service         LowCardinality(String),
...
CREATE MATERIALIZED VIEW IF NOT EXISTS observability.logs_rollup_v2_1m_to_5m
TO observability.logs_rollup_v2_5m AS
SELECT team_id,
...
FROM observability.logs_rollup_v2_1m
GROUP BY team_id, bucket_ts, severity_bucket, service, environment, host;

CREATE TABLE IF NOT EXISTS observability.logs_rollup_v2_1h (
    team_id         UInt32 CODEC(T64, ZSTD(1)),
    bucket_ts       DateTime CODEC(DoubleDelta, LZ4),
    severity_bucket UInt8,
    service         LowCardinality(String),
...
CREATE MATERIALIZED VIEW IF NOT EXISTS observability.logs_rollup_v2_5m_to_1h
TO observability.logs_rollup_v2_1h AS
SELECT team_id,
       toStartOfHour(bucket_ts) AS bucket_ts,
       severity_bucket, service, environment, host,
       sumMergeState(log_count)   AS log_count,
       sumMergeState(error_count) AS error_count
FROM observability.logs_rollup_v2_5m
GROUP BY team_id, bucket_ts, severity_bucket, service, environment, host;
