-- metrics_k8s — Kubernetes-specific gauges (k8s.pod.*, container.*,
-- k8s.container.*). state_dim is populated by the raw MATERIALIZED column;
-- MV is pure GROUP BY + state. PK leads with namespace because drill-down
-- flow is namespace → workload → pod.

CREATE TABLE IF NOT EXISTS observability.metrics_k8s_1m (
    team_id        UInt32 CODEC(T64, ZSTD(1)),
    bucket_ts      DateTime CODEC(DoubleDelta, LZ4),
    namespace      LowCardinality(String),
    metric_name    LowCardinality(String),
    service        LowCardinality(String),
    pod            LowCardinality(String),
    container      LowCardinality(String),
    host           LowCardinality(String),
    state_dim      LowCardinality(String),
    value_sum      AggregateFunction(sum, Float64),
    value_avg_num  AggregateFunction(sum, Float64),
    sample_count   AggregateFunction(sum, UInt64),
    value_max      AggregateFunction(max, Float64),
    value_min      AggregateFunction(min, Float64),
    value_last     AggregateFunction(argMax, Float64, DateTime64(3))
) ENGINE = AggregatingMergeTree()
PARTITION BY toYYYYMM(bucket_ts)
ORDER BY (team_id, bucket_ts, namespace, metric_name, service, pod, container, host, state_dim)
TTL bucket_ts + INTERVAL 72 HOUR DELETE
SETTINGS index_granularity = 8192;

CREATE MATERIALIZED VIEW IF NOT EXISTS observability.metrics_k8s_raw_to_1m
TO observability.metrics_k8s_1m AS
SELECT
    team_id,
    toStartOfMinute(timestamp)                            AS bucket_ts,
    k8s_namespace                                         AS namespace,
    metric_name,
    service,
    attributes.`k8s.pod.name`::String                     AS pod,
    attributes.`k8s.container.name`::String               AS container,
    host,
    state_dim,
    sumState(value)                                       AS value_sum,
    sumState(value)                                       AS value_avg_num,
    sumState(toUInt64(1))                                 AS sample_count,
    maxState(value)                                       AS value_max,
    minState(value)                                       AS value_min,
    argMaxState(value, timestamp)                         AS value_last
FROM observability.metrics
WHERE (metric_name LIKE 'k8s.%' OR metric_name LIKE 'container.%')
  AND hist_count = 0
GROUP BY team_id, bucket_ts, namespace, metric_name, service, pod, container, host, state_dim;

CREATE TABLE IF NOT EXISTS observability.metrics_k8s_5m (
    team_id        UInt32 CODEC(T64, ZSTD(1)),
    bucket_ts      DateTime CODEC(DoubleDelta, LZ4),
    namespace      LowCardinality(String),
    metric_name    LowCardinality(String),
    service        LowCardinality(String),
    pod            LowCardinality(String),
    container      LowCardinality(String),
    host           LowCardinality(String),
    state_dim      LowCardinality(String),
    value_sum      AggregateFunction(sum, Float64),
    value_avg_num  AggregateFunction(sum, Float64),
    sample_count   AggregateFunction(sum, UInt64),
    value_max      AggregateFunction(max, Float64),
    value_min      AggregateFunction(min, Float64),
    value_last     AggregateFunction(argMax, Float64, DateTime64(3))
) ENGINE = AggregatingMergeTree()
PARTITION BY toYYYYMM(bucket_ts)
ORDER BY (team_id, bucket_ts, namespace, metric_name, service, pod, container, host, state_dim)
TTL bucket_ts + INTERVAL 14 DAY DELETE
SETTINGS index_granularity = 8192;

CREATE MATERIALIZED VIEW IF NOT EXISTS observability.metrics_k8s_1m_to_5m
TO observability.metrics_k8s_5m AS
SELECT
    team_id,
    toStartOfInterval(bucket_ts, toIntervalMinute(5)) AS bucket_ts,
    namespace, metric_name, service, pod, container, host, state_dim,
    sumMergeState(value_sum)       AS value_sum,
    sumMergeState(value_avg_num)   AS value_avg_num,
    sumMergeState(sample_count)    AS sample_count,
    maxMergeState(value_max)       AS value_max,
    minMergeState(value_min)       AS value_min,
    argMaxMergeState(value_last)   AS value_last
FROM observability.metrics_k8s_1m
GROUP BY team_id, bucket_ts, namespace, metric_name, service, pod, container, host, state_dim;

CREATE TABLE IF NOT EXISTS observability.metrics_k8s_1h (
    team_id        UInt32 CODEC(T64, ZSTD(1)),
    bucket_ts      DateTime CODEC(DoubleDelta, LZ4),
    namespace      LowCardinality(String),
    metric_name    LowCardinality(String),
    service        LowCardinality(String),
    pod            LowCardinality(String),
    container      LowCardinality(String),
    host           LowCardinality(String),
    state_dim      LowCardinality(String),
    value_sum      AggregateFunction(sum, Float64),
    value_avg_num  AggregateFunction(sum, Float64),
    sample_count   AggregateFunction(sum, UInt64),
    value_max      AggregateFunction(max, Float64),
    value_min      AggregateFunction(min, Float64),
    value_last     AggregateFunction(argMax, Float64, DateTime64(3))
) ENGINE = AggregatingMergeTree()
PARTITION BY toYYYYMM(bucket_ts)
ORDER BY (team_id, bucket_ts, namespace, metric_name, service, pod, container, host, state_dim)
TTL bucket_ts + INTERVAL 90 DAY DELETE
SETTINGS index_granularity = 8192;

CREATE MATERIALIZED VIEW IF NOT EXISTS observability.metrics_k8s_5m_to_1h
TO observability.metrics_k8s_1h AS
SELECT
    team_id,
    toStartOfHour(bucket_ts) AS bucket_ts,
    namespace, metric_name, service, pod, container, host, state_dim,
    sumMergeState(value_sum)       AS value_sum,
    sumMergeState(value_avg_num)   AS value_avg_num,
    sumMergeState(sample_count)    AS sample_count,
    maxMergeState(value_max)       AS value_max,
    minMergeState(value_min)       AS value_min,
    argMaxMergeState(value_last)   AS value_last
FROM observability.metrics_k8s_5m
GROUP BY team_id, bucket_ts, namespace, metric_name, service, pod, container, host, state_dim;
